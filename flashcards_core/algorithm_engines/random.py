...
class RandomEngine(BaseAlgorithmEngine):
    ...
    def next_card(self) -> Card:
        ...
        logging.debug(f"Deck state: {self.deck.get_state()}. Unseen cards: {self.deck.unseen_cards_number()}")
        ...
        if deck_state.get(UNSEEN_FIRST) and self.deck.unseen_cards_number() > 0:
            logging.debug(f"Picking from the {self.deck.unseen_cards_number()} unseen cards")
        ...
        if deck_state.get(NEVER_REPEAT):
            ...
            while last_card_id == next_card.id:
                logging.debug(f"It's the same card as last time ({last_card_id}), retrying")
    ...
    def process_test_result(self, card: Card, result: Any) -> None:
        ...
        logging.debug(f"Creating Review for Card '{card}'' "
